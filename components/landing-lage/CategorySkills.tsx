...
const CategorySkills = ({ category }: Props) => {
  return (
    <div className="mx-10 mb-10">
      ...
      <div className=" grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 mb-6 justify-center gap-4 w-fit mx-auto">
        {category.skills.map(
          ({ name, icon: Icon }: { name: string; icon: LucideIcon }) => (
            <div
              key={name}
              className="flex flex-col gap-1 justify-center items-center my-4 bg-white dark:bg-brand-100/30 rounded-md h-24 w-30 sm:w-40 p-4"
            >
